          ...
          <ul className="flex flex-wrap gap-x-4 gap-y-0">
            {footer.links.map((link) => (
              <li key={link.label}>
                <a
                  href={link.href}
                  className="font-sans text-[0.56rem] uppercase tracking-[0.16em] text-glow-cream/60 transition-colors duration-300 hover:text-glow-cream sm:text-[0.62rem] sm:tracking-[0.2em]"
                >
                  {link.label}
                </a>
              </li>
